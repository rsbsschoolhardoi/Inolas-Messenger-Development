  ...
  return (
    <div className={`w-full min-h-[100dvh] flex flex-col font-sans transition-colors ${
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 sm:p-6">
        ...
        {/* ========================================================================= */}
        {/* TAB 3: INTERACTIVE SSO PLAYGROUND / SIMULATOR                             */}
        {/* ========================================================================= */}
        {activeTab === 'playground' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            {/* Left: Configuration & Test Controls */}
            <div className="lg:col-span-5 space-y-5">
              <div className={`p-6 rounded-2xl border ${
                ...
                <div className="flex items-center gap-2.5 mb-4">
                  ...
                  <div>
                    <h3 className="font-bold text-sm">OAuth 2.0 Simulator</h3>
                    <p className="text-xs text-neutral-500">Test the complete end-to-end login flow live</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold text-neutral-500 mb-1.5">
                      Select OAuth Client App
                    </label>
                    <select
                      value={selectedTesterAppId}
                      onChange={e => setSelectedTesterAppId(e.target.value)}
                      className={`w-full px-3.5 py-2.5 rounded-xl border text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-sky-500 ${
                        themeMode === 'dark' ? 'bg-neutral-950 border-neutral-800 text-white' : 'bg-neutral-50 border-neutral-200 text-neutral-900'
                      }`}
                    >
                      <option value="demo_app">Zenoa OAuth Application (demo_app)</option>
                      {apps.map(a => (
                        <option key={a.id} value={a.id}>
                          {a.app_name} ({a.client_id})
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder={getSelectedTesterApp()?.redirect_uris[0] || 'http://localhost:3000/auth/callback'}
                      className={`w-full px-3.5 py-2 rounded-xl border text-xs font-mono focus:outline-none focus:ring-2 focus:ring-sky-500 ${
                        themeMode === 'dark' ? 'bg-neutral-950 border-neutral-800 text-white' : 'bg-neutral-50 border-neutral-200 text-neutral-900'
                      }`}
                    />
                  </div>

                  {/* Standard "Sign In With Zenoa" Button Widget Preview */}
                  <div className="pt-2">
                    <label className="block text-xs font-bold text-neutral-500 mb-2">
                      Live "Continue with Zenoa" Button
                    </label>
                    <button
                      onClick={runSimulatedAuthorize}
                      disabled={isTesterRunning}
                      className="w-full py-3 px-4 rounded-xl bg-zinc-900 hover:bg-zinc-800 dark:bg-zinc-100 dark:hover:bg-white text-white dark:text-zinc-950 text-white font-bold text-sm shadow-lg shadow-sky-500/25 flex items-center justify-center gap-2.5 transition-all active:scale-95 disabled:opacity-50"
                    >
                      <div className="w-5 h-5 rounded-md bg-white text-sky-600 flex items-center justify-center font-black text-xs">
                        Z
                      </div>
                  ...
                  <div className="pt-3 border-t border-neutral-200 dark:border-neutral-800 space-y-2">
                    <button
                      ...
                      className="w-full py-2 px-3 rounded-lg border border-sky-500/30 text-sky-600 dark:text-sky-400 hover:bg-sky-500/10 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors disabled:opacity-30"
                    >
                      <Key className="w-3.5 h-3.5" />
                      <span>Step 2: Exchange Code for Access Token</span>
                    </button>

                    <button
                      onClick={runSimulatedUserInfoFetch}
                      disabled={!playgroundAccessToken || isTesterRunning}
                      className="w-full py-2 px-3 rounded-lg border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors disabled:opacity-30"
                    >
                      <UserCheck className="w-3.5 h-3.5" />
                      <span>Step 3: Fetch Verified User Profile</span>
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className={`p-4 rounded-2xl border ${
                themeMode === 'dark' ? 'bg-neutral-900/40 border-neutral-800' : 'bg-neutral-100 border-neutral-200'
              }`}>
                <div className="flex items-center justify-between text-xs font-semibold text-neutral-500 mb-3">
                  <span>OAuth 2.0 Pipeline Flow</span>
                  <span className="font-mono text-[11px]">
                    {playgroundStep === 'idle' ? 'Ready' : playgroundStep}
                  </span>
                </div>

                <div className="flex items-center justify-between gap-1 text-[10px] font-bold">
                  <div className={`flex-1 p-2 rounded-lg text-center border ${
                    playgroundStep !== 'idle'
            ...
            {/* Right: Live Execution Logs & Returned Data */}
            <div className="lg:col-span-7 space-y-4">
              ...
              <div className={`rounded-2xl border overflow-hidden ${
                ...
                <div className="px-4 py-3 bg-neutral-900 border-b border-neutral-800 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono font-semibold text-neutral-300">Live OAuth Handshake Log</span>
                  </div>
                  <span className="text-[10px] font-mono text-neutral-500">Real-Time Output</span>
                </div>
                <div className="p-4 font-mono text-xs space-y-1.5 min-h-[160px] max-h-[220px] overflow-y-auto">
                  {testerLog.length === 0 ? (
                    <div className="text-neutral-500 italic">Click "Continue with Zenoa" to execute the OAuth flow...</div>
                  ) : (
                    testerLog.map((log, index) => (
                      <div
                        key={index}
                        className={
                          log.startsWith('✓')
              ...
              {/* User Profile Extraction View */}
              {playgroundUserResult && (
                <motion.div
                  initial={{ opacity: 0, scale: 0.98 }}
                  animate={{ opacity: 1, scale: 1 }}
                  className={`p-5 rounded-2xl border ${
                    themeMode === 'dark' ? 'bg-emerald-950/20 border-emerald-500/30' : 'bg-emerald-50 border-emerald-200'
                  }`}
                >
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 rounded-xl bg-emerald-500/20 text-emerald-600 flex items-center justify-center font-bold text-lg">
                      ✓
                    </div>
                    <div>
                      ...
                      <p className="text-xs text-neutral-500">
                        Ready to be saved to your application's session or database
                      </p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                    <div className="p-2.5 rounded-lg bg-white/40 dark:bg-neutral-900/40 border border-emerald-500/20">
                      <span className="text-[10px] text-neutral-500 block">Username</span>
                      <span className="font-bold">@{playgroundUserResult.username}</span>
                    </div>
                    <div className="p-2.5 rounded-lg bg-white/40 dark:bg-neutral-900/40 border border-emerald-500/20">
                      <span className="text-[10px] text-neutral-500 block">Display Name</span>
                      <span className="font-bold">{playgroundUserResult.display_name}</span>
                    </div>
                    <div className="p-2.5 rounded-lg bg-white/40 dark:bg-neutral-900/40 border border-emerald-500/20">
                      <span className="text-[10px] text-neutral-500 block">Phone</span>
                      <span className="font-bold">{playgroundUserResult.mobile_number || 'N/A'}</span>
                    </div>
                    <div className="p-2.5 rounded-lg bg-white/40 dark:bg-neutral-900/40 border border-emerald-500/20">
                      <span className="text-[10px] text-neutral-500 block">Email</span>
                      <span className="font-bold truncate block">{playgroundUserResult.email || 'N/A'}</span>
                    </div>
                  </div>
                </motion.div>
              )}
